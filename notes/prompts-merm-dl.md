...
<head>
    ...
    <style>
        ...
        .graph-options { margin-bottom: 10px; }
        .graph-options label { display: inline-block; margin-right: 5px; font-weight: normal; }
        .graph-options select { width: auto; display: inline-block; padding: 5px; }
...
<body>
    ...
    <script>
        ...
        let currentSentenceTokensForGraph = []; // Stores tokens for current sentence graph
        let currentGraphDirection = 'BT'; // Default graph direction
        ...
        function clearPreviousData() {
            ...
            currentSentenceTokensForGraph = []; // Clear stored tokens
            // currentGraphDirection = 'BT'; // Optionally reset direction
            // graphDirectionSelector.value = 'BT'; // Optionally reset selector UI
        ...
        async function renderMermaidGraph(tokensToGraph, direction) {
            mermaidGraphContainer.innerHTML = ''; // Clear previous graph
            mermaidGraphContainer.removeAttribute('data-processed');
            ...
            const mermaidDef = [`graph ${direction};`]; // Use dynamic direction
        ...
        async function handleSentenceSelection(event) {
            ...
            currentSentenceTokensForGraph = []; // Clear for new sentence
            ...
            if (VUsInSent.length === 0) {
                ...
                await renderMermaidGraph([], currentGraphDirection); // Render empty graph message
                return;
            }
            ...
            currentSentenceTokensForGraph = sentToks; // Store for re-renders
